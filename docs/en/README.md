# EppTools for Java — reference manual
...
manual is the wire protocol of **RFC 5730–5734** with a Java object in front of it: a session, a
...
The manual assumes you can write Java 8 or newer and that EPP is new to you. Where a method
...
- **Dates and money come back as the registry's own strings**, never as `Instant` or `double`. The
...
Java 8 or newer. The published jar is compiled with `--release 8`, so one artifact runs on 8, 11,
17, 21 and every later release. It uses `javax.net.ssl` for TLS and `javax.xml` for DOM, both part
of the JDK, and depends on nothing else — no XML library, no logging facade, no HTTP client. Every
exception it throws is unchecked, so no method signature carries a `throws`.
...
Include the library version as well — `com.epptools.sdk.Version.VERSION`, which is the same string
this client reports to the server in its login (RFC 8807 `<loginSec:app>`), so the version in your
report and the version the registry saw are the one version:
