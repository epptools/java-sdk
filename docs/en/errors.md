...
Every failure this library throws extends `com.epptools.sdk.exception.EppException`, so one `catch`
handles everything. Beyond that, **a class exists where the right next step differs — and nowhere
else.** The taxonomy is not a description of what went wrong; it is a description of what to do about
it.
...
`EppException` extends `RuntimeException`, so a catch-all higher up your stack still sees it — and so
**nothing in a method signature warns you that a command can fail.** There is no `throws` to
acknowledge, no compiler nagging you into a `try`. That is deliberate: a checked exception on every
command would put a `try` around every line of a provisioning routine and teach people to write empty
catch blocks. The cost is that the `try` above is yours to remember.

## The hierarchy

```
EppException  (extends RuntimeException)
...
| Catch | Thrown when | Codes | What to do next |
|---|---|---|---|
...
Catch from the specific to the general. Here the compiler is on your side: a `catch` for a subclass
written **after** one for its superclass does not compile — *"exception has already been caught"* —
so you cannot silently swallow the one you meant to treat differently. It will not, however, stop you
writing a single `catch (EppException)` where you needed two.
...
The key check reaches **inside** the nested maps too, so `emial` in a `disclose` block, `key_tag` in a
`dsData` record, `curency` in a fee agreement, `nmae` in a glue entry and `postalCode` in a
`postalInfo` are each refused with the nearest accepted spelling named. That matters most where the
frame the misspelling builds is **schema-valid**: a DS record of zeros with an empty digest is
accepted, answered 1000, and published as a signer matching no key.

Others of the same kind: a mixture of the two nameserver models in one command, `dsData` mixed with
`keyData` in one secDNS block, a `maxSigLife` under one second, a fee amount that is not a plain
decimal, an empty contact role or one outside admin/billing/tech, a transfer `op` outside the five RFC
5730 names, an `info` `hosts` outside all/del/sub/none, a postal `type` or a disclosed field’s form
outside int/loc, an empty e-mail on a contact create, a `check()` with no names at all, a fee query
that names a currency and no operation, a `newName` on a host update (a registry that discards
`host:chg`), `authInfo` and `clearAuthInfo` in the same `chg`, `removeAllDnssec()` combined with
removing a named record, a postal change missing its name, city or country code, a fee query over
twenty entries, and a second `send()` on a builder.
...
The host and the clID are checked when the `Config` is built, so a missing one is a `ConfigException`
out of `build()` before a `Client` exists at all; `connect()` re-checks the host as a guard and can
report it too. The password length rules arrive later, and at two moments. The bounds that hold for
...
|---|---|
| `Cannot connect to epp.registry.example:700 - …` | the TCP connect failed. The tail is the reason the JDK reported, not a generic one |
| `TLS handshake with epp.registry.example:700 failed - PKIX path building failed…` | `caFile` is unset or points at the wrong bundle |
| `Read timed out` | `readTimeout` elapsed with the command in flight |
...
| `Connection is no longer usable: …` | an earlier read or write failed and latched; the tail is that first failure |
| `Not connected` / `Not connected - call connect() first` | no socket; the session was never opened, or was closed by an earlier failure |
| `Server returned malformed XML: …` | the frame did not parse |
| `Server returned XML with a DOCTYPE - refused` | an EPP response never carries one; refusing it closes the XXE door |
...
silent: `isConnected()` goes false, the failure is latched, and every later call refuses rather than
reading from the middle of a frame.
...
```java
int eppCode();               // the EPP result code
Response response();         // the full parsed reply, or null when none was captured
boolean isRetryable();       // whether sending the very same command again could succeed
String subject();            // the object the registry objected to, when it named one
List<String> reasons();      // the extra diagnostic prose
```

| Member | Returns |
|---|---|
| `e.eppCode()` | the result code as an `int` |
| `e.response()` | the whole [`Response`](responses.md) — everything the reply carried |
| `e.isRetryable()` | whether sending the very same command again could succeed |
| `e.subject()` | the object the registry objected to, when it named one |
| `e.reasons()` | the extra diagnostic prose, as a `List<String>` |
| `CommandException.forCode(int code, String message, Response response)` | builds the most specific subclass for a code |
...
`response()` is `null` only when no reply was parsed. Everywhere else it is the full response —
`errorReasons()`, `extValues()`, `svTRID()` and the raw frame are all there for a log. It is a
`transient` field, so an exception you serialise and read back has the code and the message but not
the response; log what you need from it before it crosses that boundary.
...
A login failure is thrown **whatever `throwOnFailure` says**, because a login that failed is not a
...
try {
    ...
    retryLater.add("example.com.ua");       // and back off before you try
}
```
...
By default every code ≥ 2000 is thrown. That is what makes a straight-line integration correct by
...
- The `CommandException` `poll().drain()` throws when a reply is neither a notice nor an empty queue.
...
`com.epptools.sdk.ResultCode` has a named constant for every code. Branch on those rather than on
bare numbers: `e.eppCode() == ResultCode.OBJECT_EXISTS` says what it means.
...
nothing is thrown — and the object is *not* yet created, renewed or transferred. Test `isPending()`
...
try {
    ...
    } catch (ObjectDoesNotExistException gone) {
...
- **Reconnect before reconciling.** The connection that lost the reply was closed by the failure and
  latched as unusable, so every call on it would throw; and a stream at an unknown offset could not be
  trusted to answer the question you are about to ask even if it were open.
